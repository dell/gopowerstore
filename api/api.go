...
func New(apiURL string, username string,
	...
	log.Printf("Bharath - NewTimeoutSemaphore: %d, %d", defaultTimeout, time.Duration(defaultTimeout)*time.Second)
...
func (c *ClientIMPL) setupContext(ctx context.Context) (context.Context, *func()) {
	...
	if !timeoutIsSet {
		var f func()
		c.logger.Info(ctx, "In setupContext - default timeout: %d - duration: %d", c.defaultTimeout, time.Duration(c.defaultTimeout)*time.Second)
